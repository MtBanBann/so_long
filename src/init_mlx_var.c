...
/*   Updated: 2024/03/19 12:28:18 by afavier          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "so_long.h"

static void	vars_put_img(t_vars *new_vars, t_map *map, int img_width,
				int img_height);
static void	vars_put_img(t_vars *new_vars, t_map *map, int img_width,
				int img_height);
...
t_vars	*initialize_vars(t_map *map)
{
	...
	new_vars->win = mlx_new_window(new_vars->mlx, (map->width - 1)
			* 100, map->height * 100, "so long");
...
static void	vars_put_img_following(t_vars *new_vars, t_map *map)
{
